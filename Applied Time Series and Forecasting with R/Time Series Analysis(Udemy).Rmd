...
Infomration criteria is not a sufficinet base to compare different systems of models. Forecast accuracy is measured by
mean square error (MSE). We use function `tsCV()` from the `forecast`package. 
...
3. Generage the mean of the error rates
...
- Getting the most accurate model possible with `stepwise=T` and 
`approximation=F`
```{r}
forecastarima=
  function(x,h)
